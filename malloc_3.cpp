...
/**
 * Intro:
 * In this implementation we'll distinguish between allocation size smaller than 128KB, and 128KB or higher.
 * The first one will be done using Smalloc() (means using sbrk), ours equivalent to malloc, and the other one using Mmap().
 * We'll manage a sorted list of addresses of our allocated blocks (virtual addresses of course),
 * as well as histogram for the free blocks. Each histogram cell (Bin) will point to linked list of free blocks (allocated before and freed by user).
 * In order to minimize inner fragmentation we'll split allocated block if the unused part of it is at least MIN_SPLIT_SIZE (see above).
 * In order to minimize external fragmentation we would combine adjacent free blocks together to one big free block.
 * In addition, in each allocation we would first search through our histogram of free blocks for the best fit of free block before
 * increasing the brk.
 */

/**
 *Struct which contains additional data required for block allocation. 
 */
...
class SbrkBlockList{
    ...
    public:
        ...
        MallocMetadata* findBySizeHist(size_t size)
        {
            ...
            while(ptr!=NULL && ptr->size-sizeof(MallocMetadata) <size)
                ptr=ptr->histo_next;
            return ptr;
        }
...
class BlockManager
{
    ...
    void Split(MallocMetadata* ptr,size_t size)
    {
        ...
        md_new_free->addr=(void*)((long)(md_new_free)+(long)(sizeof(MallocMetadata)));
        ...
        histogram[IndexOfHisto(md_new_free->size-sizeof(MallocMetadata))].insertBySizeHisto(md_new_free);    
    }
    ...
    void* BlockAllocate(size_t size)
    {
        ...
        {
            MallocMetadata* meta_data_ptr=(MallocMetadata*)(mmap(NULL, size+sizeof(MallocMetadata), PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0));
            if(meta_data_ptr==(MallocMetadata*)(-1))
                    return NULL;
            else{
                    meta_data_ptr->addr=(void*)((long)meta_data_ptr+(long)sizeof(MallocMetadata));
                    meta_data_ptr->is_free=false;
                    meta_data_ptr->size=size+sizeof(MallocMetadata);
            ...
            **/
            {
                if(sbrk(real_size - free_tail->size)==(void*)-1)
                    return NULL;
                histogram[IndexOfHisto(free_tail->size-sizeof(MallocMetadata))].removeHisto(free_tail);
    ...
    void FreeBlock(void* addrs)
        ...
        **/
    {
        MallocMetadata* md_to_free=(MallocMetadata*)((long)addrs-sizeof(MallocMetadata));
        if(md_to_free->size-sizeof(MallocMetadata)>MMAP_ALLOCATION_MIN_SIZE) //this block is of mmap, should use unmap
        {
            mmap_allocated_blocks--;
            size_t data_size=(size_t)((size_t)md_to_free->size - (size_t)sizeof(MallocMetadata));
            ...
            histogram[IndexOfHisto(md_to_free->size-sizeof(MallocMetadata))].insertBySizeHisto(md_to_free);
    ...
    MallocMetadata* Merge(MallocMetadata* first_block,bool free, bool mirror)    
    {
        ...
        histogram[IndexOfHisto(md_to_free->size-sizeof(MallocMetadata))].removeHisto(md_to_free);
        first_block->size+=second_block->size;
        if(free)
        {
            histogram[IndexOfHisto(second_block->size-sizeof(MallocMetadata))].removeHisto(second_block);
            histogram[IndexOfHisto(first_block->size-sizeof(MallocMetadata))].insertBySizeHisto(first_block);
        }
        else
    ...
    {
        MallocMetadata* md_to_realloc=(MallocMetadata*)((long)oldp-sizeof(MallocMetadata));
        if(md_to_realloc->size-sizeof(MallocMetadata) > MMAP_ALLOCATION_MIN_SIZE) //should use mmap
        {
            if(size == (md_to_realloc->size -sizeof(MallocMetadata)))
                return oldp;
            MallocMetadata* meta_data_ptr=(MallocMetadata*)(mmap(0, size+sizeof(MallocMetadata), PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0));
            if(meta_data_ptr==(MallocMetadata*)(-1))
                    return NULL;
            mmap_allocated_bytes -= (md_to_realloc->size-sizeof(MallocMetadata));
            mmap_allocated_bytes += size;
            meta_data_ptr->addr=(void*)((long)meta_data_ptr+(long)(sizeof(MallocMetadata)));
            size_t amount_to_cpy= (size<(md_to_realloc->size-sizeof(MallocMetadata)) ? size : (md_to_realloc->size-sizeof(MallocMetadata)));
        ...
        size_t real_size=size+sizeof(MallocMetadata);
        size_t old_size = md_to_realloc->size-sizeof(MallocMetadata);
        if(md_to_realloc->size-sizeof(MallocMetadata) >= size)
        {
            if(md_to_realloc->size-real_size >= MIN_SPLIT_SIZE+sizeof(MallocMetadata))
                Split(md_to_realloc, real_size);
            return oldp;
        }
        else if(all_blocks_list.IsPrevFree(md_to_realloc) && md_to_realloc->size+md_to_realloc->list_prev->size-sizeof(MallocMetadata) > size)
        {
            md_to_realloc=Merge(md_to_realloc->list_prev,false,false);
            if((long)(md_to_realloc->size-real_size-sizeof(MallocMetadata))>=MIN_SPLIT_SIZE)
                Split(md_to_realloc, real_size);
            memmove(md_to_realloc->addr,oldp,old_size);
            return md_to_realloc->addr;
        }
        
        else if(all_blocks_list.IsNextFree(md_to_realloc) && md_to_realloc->size+md_to_realloc->list_next->size-sizeof(MallocMetadata) > size)
        {
            ...
            return md_to_realloc->addr;
                ...
                && md_to_realloc->size+md_to_realloc->list_next->size+md_to_realloc->list_prev->size-sizeof(MallocMetadata) > size)
        {
            md_to_realloc=Merge(md_to_realloc->list_prev,false,false);
            md_to_realloc=Merge(md_to_realloc,false,true);
            if((long)(md_to_realloc->size-real_size-sizeof(MallocMetadata))>=MIN_SPLIT_SIZE)
    ...
    {
        return all_blocks_list.numMetaDataBytesList()+mmap_allocated_blocks*sizeof(MallocMetadata);
    }
...
/**
 * Our implementation for malloc C function.
 * @param:
 *          size - size user required to allocate to data.
 * @return:
 *          on successful allocation, return a pointer to address.
...
/**
 * Our implementation for calloc C function.
 * Like the origin, put zero in the data addresses, size of (size*num).
 * @param:
 *          size - size of data object user required.
 *          num - number of allocated objects user required.
 * @return:
 *          on successful allocation, return a pointer to address.
 *          on failure, return NULL.
 */
...
/**
 * Our implementation for free C function.
 * @param:
 *          p - pointer to an adress.
 */
void sfree(void* p)
{
    if(p==NULL)
...
/** UTILITY FUNCTIONS
 * Used for verification and staff tests.
 **/
